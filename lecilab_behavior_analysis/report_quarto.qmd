...
#tv_projects[1] is out of range, changed to explicit visual_and_COT_data and mouse selection
...
local_path = Path(utils.get_outpath()) / Path("visual_and_COT_data") / Path("sessions") / Path(mouse)
df = pd.read_csv(local_path / Path(f'{mouse}.csv'), sep=";")
fig1 = subject_progress_figure(df, perf_window=100, summary_matrix_plot=False)
plt.close('all')
fig1
```
```{python}
date = "2025-06-25"
df_summary = add_day_column_to_df(df)
df_summary = add_trial_of_day_column_to_df(df_summary)
sdf = df_summary[df_summary["year_month_day"] == date]
try:
...
try:
    ...
    print(f"Plotting failed due to missing key: {e}. Skipping figure for {date}.")
```
